...
MunitResult test_new_config(const MunitParameter params[],
  ...
  check(bstrcmp(conf->pki,bfromcstr("<path>.pem"))==0,"PKI has wrong value: %s",bdata(conf->pki));
  check(bstrcmp(conf->cert,bfromcstr("<path>.pem"))==0,"Cert has wrong value: %s",bdata(conf->cert));
...
int main(int argc, char *argv[]) {
  MunitTest tests[] = {
  {" test_new_config",test_new_config,NULL, NULL, MUNIT_TEST_OPTION_NONE, NULL},
